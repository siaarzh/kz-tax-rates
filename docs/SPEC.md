...
```csv
kato,kato_version,name_ru,name_kk,rate,base_rate,valid_from,valid_to,decision_ref,source_url,extraction_method
750000000,NK RK 11-2025,г. Алматы,Алматы қаласы,0.03,0.04,2026-01-01,2026-12-31,Решение маслихата г. Алматы №256 от 28.11.2025,https://adilet.zan.kz/rus/docs/...,deterministic-readers
```
...
| `extraction_method` | string | How the number was obtained, e.g. `deterministic-readers`. **Required.** |
...
>
> **§7 below is unchanged and still governs**, except §7.1: there is no human merge gate. `data/rates.csv` is generated by `scripts/publish_rates.py` from `data/mapped-rates.json`, and what protects a row is the citation, the deterministic reading, the regime check and the cross-check against the mapper's own current conclusion — never a person's approval.
...
### 7.1 SUPERSEDED — there is no human merge gate

> Nobody hand-confirms a row. `data/rates.csv` is generated by `scripts/publish_rates.py`
> straight from `data/mapped-rates.json`, with no pull request and no approving review in
> between. What protects a row is not a person's approval: it is the citation
> (`source_url`, `decision_ref`), the deterministic reading (two independent readers of two
> different-language files agreeing), the regime check, and the test that every published
> row still matches the mapper's own current conclusion, per year, in both directions
> (`test_published_rows_match_the_currently_mapped_set_exactly`). §7.2 through §7.5 below are
> unaffected: nothing here reintroduces a second model checking the first.
...
- `extraction_method` is not empty
...
- **Never write a rate you inferred.** Every rate must be read out of the decision by a
  deterministic reader. If you cannot cite `source_url`, do not write the row.
- **`data/rates.csv` has one writer: `scripts/publish_rates.py`, from `data/mapped-rates.json`.**
  There is no human merge gate and no PR ceremony for a data change — the protection is the
  citation, the deterministic reading and the cross-check against the mapper, not an approval.
